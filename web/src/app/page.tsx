    ...
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <div id="ad-banner" className="bg-gradient-to-r from-indigo-600 to-blue-500 text-white text-center py-2.5 px-4 text-sm font-medium">
        <a href="https://www.eduland.or.kr" target="_blank" rel="noopener noreferrer" className="hover:underline">
          에듀랜드 공공조달관리사 — 합격 전략 무료 공개 중
        </a>
      </div>

      {/* Hero Section */}
      <header className="relative overflow-hidden bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#1e3a5f]">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              ...
              <p className="mt-1.5 text-sm sm:text-base text-transparent bg-clip-text bg-gradient-to-r from-sky-300 to-teal-300 font-medium">
                by 에듀랜드
              </p>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        {/* Search & Filter */}
        <div className="mb-10">
          <SearchWrapper basePath="/procurement-wiki" />
        </div>
        ...
        <section className="mb-14">
          ...
          {(() => {
            const overview = practice.filter((p) => p.frontmatter.title.includes("종합") || p.frontmatter.title.includes("분석"));
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-2">
          <span className="text-xs text-slate-400">에듀랜드 위키 · 공공조달관리사 시험 대비</span>
          <span className="text-xs text-slate-300">교재 기반 체계적 학습 자료</span>
        </div>
      </footer>
    </div>
  );
